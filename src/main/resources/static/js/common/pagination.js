...
(function($, thisPage) {
    ...
    const PAGE_SIZE = thisPage.pageSize || 10;
    
    /** page range block(default : 10) **/
    const PAGE_BLOCK = thisPage.pageBlock || 10;
    
    /** template table list && pagination **/
    $.fn.template = function(result, callback, pageNo, orderNo) {
        const page = pageNo || 1;
        const order = orderNo || 0;
        
        // template table list
        const currentResult = getOffsetList(result, page);
        const listHtml = callback(currentResult);
        this.html(listHtml);
        
        // template pagination
        const pagingHtml = getPagination(result.length, page, order);
     ...
     */
    function getOffsetList(resultArr, pageNo) {
        -- pageNo;
    ...
    function getPagination(totalRow, pageNo, orderNo) {
        var page = pageNo || 1;
        var order = orderNo || 0;
